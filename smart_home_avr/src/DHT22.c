...
DHT22_ERROR_t readDHT22( uint8_t *dht_humidity_now, float *dht_temperature_now )
{
	...
	retryCount = 0;
	cli();
	DHT_DDR &= ~(1 << ( DHT_PIN )); 							//DIRECT_MODE_INPUT(reg, bitmask);
	sei();
	do
	{
		...
		retryCount++;
		_delay_us(2);
	} while( !( DHT_IN & ( 1 << DHT_PIN ) ) );				//!DIRECT_READ(reg, bitmask)

	// Send the activate pulse
	cli();
	PORTC &= ~(1 << ( DHT_PIN )); 							//DIRECT_WRITE_LOW(reg, bitmask);
	DHT_DDR |= 1 << ( DHT_PIN );								//DIRECT_MODE_OUTPUT(reg, bitmask); // Output Low
	sei();
	_delay_ms(20); 										// spec is 1 to 10ms
	cli();
	DHT_DDR &= ~(1 << ( DHT_PIN ));;							// Switch back to input so pin can float
	sei();
